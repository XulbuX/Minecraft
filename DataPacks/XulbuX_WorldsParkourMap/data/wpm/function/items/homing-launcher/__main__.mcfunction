# DEFINE & SHOW AMMUNITION
   ...
   execute as @a[scores={homing_missile=1..},nbt={SelectedItem:{components:{"minecraft:custom_data":{homing_missile_launcher:1}}}}] run execute unless entity @s[scores={homing_ammo=..0}] run scoreboard players remove @s homing_ammo 1
...
# LAUNCH HOMING-MISSILE WITH DEFINED LAUNCHER
   execute as @a[scores={homing_missile=1..},nbt={SelectedItem:{components:{"minecraft:custom_data":{homing_missile_launcher:1}}}}] run function wpm:items/homing-launcher/launch
