<template>
  <div class="flex flex-wrap select-none items-center gap-x-4 gap-y-3 border border-b-0 border-white/10 rounded-t-lg bg-gray-8 px-3.5 py-3">
    <div class="flex gap-1">
      <button
        class="format-b size-6 flex items-center justify-center border border-white/10 rounded bg-gray-7 text-lg leading-none duration-200 -tracking-0.5 hover:brightness-125"
        :class="{ 'bg-gray-6! font-bold border-gray-5!': editor?.isActive('bold') }"
        @click="toggleBold">
        B
      </button>
      <button
        class="format-i size-6 flex items-center justify-center border border-white/10 rounded bg-gray-7 text-lg leading-none tracking-0.2 duration-200 hover:brightness-125"
        :class="{ 'bg-gray-6! font-bold border-gray-5!': editor?.isActive('italic') }"
        @click="toggleItalic">
        I
      </button>
      <button
        class="format-u size-6 flex items-center justify-center border border-white/10 rounded bg-gray-7 text-lg leading-none duration-200 -tracking-0.2 hover:brightness-125"
